...
def cwr_loss(outputs, targets, d=0.5, eps=1e-12):
   
    logp = F.log_softmax(outputs, dim=1)
    log_fy = logp[torch.arange(targets.shape[0], device=targets.device), targets]
    log_frej = logp[:, -1]
    loss = -(log_fy + (1.0 - d) * log_frej)
    return loss.mean()
